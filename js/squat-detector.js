/* =====================================================================
   SQUAT DETECTOR — v0.3 (automatic acquisition, no stillness step)
   ---------------------------------------------------------------------
...
const SquatDetector = (function () {
  ...
  const CONFIG = {
    ...
    readyHoldMs: 120,           // brief evidence window; movement is allowed
    readyMinFrames: 3,
  ...
  function readySample(kp, visibility, side) {
    ...
    if (thighLength < 0.01) return null;
    const kneeAngle = hasAnkle ? angleAt(hip, knee, kp[prefix + 'ankle']) : null;
    // Observe the upper part of a cycle naturally. No trunk alignment,
    // camera-facing orientation or fixed screen position is required.
    const upper = hasAnkle
      ? kneeAngle >= CONFIG.descendEnterAngle
      : (knee[1] - hip[1]) / thighLength >= 0.7;
    return { side, hasAnkle, upper, kneeAngle, thighLength,
      relativeHip: [hip[0] - knee[0], hip[1] - knee[1]],
      scale: (torsoLength || thighLength) + thighLength };
  }
  ...
  function updateReadiness(kp, visibility, now) {
    ...
    const sample = samples.find(s => s.side === previousSample?.side) || samples[0];
    if (!sample) {
      readyCandidate = null;
      return;
    }
    ...
    // Abrupt leg collapse / scale change is not a squat. Reacquire the
    // cycle automatically; coherent camera/body translation cancels out.
    const comparable = previous && previous.side === sample.side &&
      dt > 0 && dt <= CONFIG.readyMaxFrameGapMs;
    const discontinuity = comparable && (
      Math.abs(sample.thighLength - previous.thighLength) > previous.thighLength * 0.4 ||
      Math.hypot(sample.relativeHip[0] - previous.relativeHip[0],
        sample.relativeHip[1] - previous.relativeHip[1]) >
        previous.scale * Math.max(0.22, dt / 1000 * 3) ||
      (sample.hasAnkle && previous.hasAnkle &&
        Math.abs(sample.kneeAngle - previous.kneeAngle) > Math.max(25, dt * 0.5))
    );
    if (discontinuity) waitForStanding();
    previousSample = { ...sample, time: now };
    if (ready) return;
    if (!sample.upper || discontinuity) {
      readyCandidate = null;
      return;
    }
    const candidate = readyCandidate;
    const sameWindow = candidate && candidate.side === sample.side &&
      candidate.hasAnkle === sample.hasAnkle &&
  ...
  function update(pose, visibility, timestamp) {
    ...
    const frameFactor = lastUpdateTime === null ? 1
      : Math.max(0.25, Math.min(3, (now - lastUpdateTime) / (1000 / 30)));
    const smoothingAlpha = 1 - Math.pow(1 - CONFIG.smoothingAlpha, frameFactor);
    const referenceAlpha = 1 - Math.pow(1 - CONFIG.standingReferenceAlpha, frameFactor);
    ...
    updateReadiness(kp, visibility, now);
    ...
    if (ready && partialParts.length) {
      ...
      smoothedHipY = ema(smoothedHipY, hipY, smoothingAlpha);

      // Follow small idle shifts, but do not move the reference down along
      // with an actual descent (especially on high-frame-rate cameras).
      if (state === 'standing' && (standingHipY === null ||
          Math.abs(smoothedHipY - standingHipY) < thighLength * 0.08)) {
        standingHipY = ema(standingHipY, smoothedHipY, referenceAlpha);
        standingThighLength = ema(standingThighLength, thighLength, referenceAlpha);
      }
    ...
    if (combinedAngle !== null) {
      ...
      framesWithoutData = 0;
      smoothedAngle = justReady ? combinedAngle : ema(smoothedAngle, combinedAngle, smoothingAlpha);
  ...
  return { update, reset, resetTracking, configure, getConfig };
})();
