/* =====================================================================
   SQUAT DETECTOR — v0.3 (immediate tracking, complete cycles only)
   ---------------------------------------------------------------------
...
const SquatDetector = (function () {
  ...
  const CONFIG = {
    ...
    smoothingAlpha: 0.5,    // EMA factor for the combined knee angle (0..1, higher = more responsive/less smooth)
    ...
    modeGraceMs: 150,          // brief ankle loss does not flip the movement signal
  ...
  let hasStart = false;        // one upper-position observation, never a stillness timer
  let references = {};
  ...
  function angleAt(a, b, c) {
    ...
    const v1x = (a[0] - b[0]) * aspectRatio, v1y = a[1] - b[1];
    const v2x = (c[0] - b[0]) * aspectRatio, v2y = c[1] - b[1];
  ...
  function hipKneeMeasurement(kp, visibility, side) {
    ...
    const thighLength = Math.hypot(
      (hip[0] - knee[0]) * aspectRatio,
      hip[1] - knee[1]
    );

    return thighLength > 0.001
      ? { side, hipY: hip[1], thighLength, upright: (knee[1] - hip[1]) / thighLength > 0.65 }
      : null;
  }
   ...
   */
  function update(pose, visibility, timestamp, geometry) {
    const now = Number.isFinite(timestamp) ? timestamp : performance.now();
    aspectRatio = Number.isFinite(geometry?.aspectRatio) && geometry.aspectRatio > 0
      ? geometry.aspectRatio : 1;
    const dt = lastUpdateTime === null ? 1000 / 30 : Math.max(1, now - lastUpdateTime);
    const alpha = 1 - Math.pow(1 - CONFIG.smoothingAlpha, Math.min(3, dt / (1000 / 30)));
    ...
    // Establish each side independently: changing which leg is visible must
    // not change the hip baseline. Never chase a descending hip downward.
    const upperPosition = combinedAngle !== null
      ? combinedAngle >= CONFIG.descendEnterAngle
      : partialParts.some(p => p.upright);
    if (state === 'standing' && upperPosition) hasStart = true;
    const drops = [];
    for (const part of partialParts) {
      let ref = references[part.side];
      if (!ref && state === 'standing' && upperPosition) {
        ref = references[part.side] = { hipY: part.hipY, scale: part.thighLength, filteredY: part.hipY };
      }
      if (!ref) continue;
      ref.filteredY = ema(ref.filteredY, part.hipY, alpha);
      if (state === 'standing' && upperPosition && ref.filteredY <= ref.hipY) {
        ref.hipY = ema(ref.hipY, ref.filteredY, CONFIG.standingReferenceAlpha);
        ref.scale = ema(ref.scale, part.thighLength, CONFIG.standingReferenceAlpha);
      }
      drops.push(Math.max(0, (ref.filteredY - ref.hipY) / ref.scale));
    }
    const partialMotion = drops.length ? drops.reduce((a, b) => a + b, 0) / drops.length : null;

    let confidence = 0;
    let mode = 'unavailable';
    let dataStale = false;
    fullFrames = combinedAngle !== null ? fullFrames + 1 : 0;
    if (combinedAngle !== null) lastFullTime = now;
    const useFull = combinedAngle !== null &&
      (activeMode !== 'partial' || fullFrames >= CONFIG.confirmFrames);
    const usePartial = partialMotion !== null &&
      (activeMode === 'partial' || lastFullTime === null || now - lastFullTime >= CONFIG.modeGraceMs);

    if (useFull) {
      mode = 'full';
      confidence = (parts.length === 2 ? CONFIG.dualLegConfidence : CONFIG.singleLegConfidence) *
        (parts.reduce((s, p) => s + p.weight, 0) / parts.length);
      smoothedAngle = activeMode === 'full' ? ema(smoothedAngle, combinedAngle, alpha) : combinedAngle;
    } else if (usePartial) {
      mode = 'partial';
      confidence = partialParts.length === 2
        ? CONFIG.partialDualSideConfidence : CONFIG.partialSingleSideConfidence;
    }
    if (mode !== 'unavailable') {
      framesWithoutData = 0;
      if (activeMode !== mode) pending = { target: null, count: 0 };
      activeMode = mode;
      if (hasStart) {
        if (mode === 'full') stepFullStateMachine(smoothedAngle);
        else stepPartialStateMachine(partialMotion);
      }
    } else {
      framesWithoutData++;
    ...
    lastResult = {
      ...
      // One unit in BOTH modes: hip descent / standing thigh projection.
      // Negative knee degrees and positive hip drop must never be compared.
      bottomMetric: mode !== 'unavailable' ? partialMotion : null,
      bottomMetricType: 'hipDrop',
