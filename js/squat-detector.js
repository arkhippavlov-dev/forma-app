/* =====================================================================
   SQUAT DETECTOR — v0.3 (immediate counting, no standing pause)
   ---------------------------------------------------------------------
...
const SquatDetector = (function () {
  ...
  const CONFIG = {
    ...
    // If tracking is lost for a while, cancel only the incomplete movement.
    // Counting starts again immediately as soon as usable tracking returns.
    reacquireAfterMs: 800
  };
  ...
  function update(pose, visibility, timestamp) {
    ...
    // No initial stillness gate. As soon as we have enough usable
    // landmarks, the detector is allowed to follow the movement.
    const ready = trackingAvailable;
    ...
    if (combinedAngle !== null) {
      ...
      stepFullStateMachine(smoothedAngle);
      ...
      framesWithoutData = 0;
      if (partialMotion !== null) stepPartialStateMachine(partialMotion);
    ...
    lastResult = {
      ...
      state: state,
      phase: phase,
      // Kept for app compatibility. There is no separate readiness lock now:
      // usable tracking means the detector may count immediately.
      ready: trackingAvailable,
      trackingAvailable: trackingAvailable,
      readiness: trackingAvailable ? 'ready' : 'waiting',
      readinessProgress: trackingAvailable ? 1 : 0,
