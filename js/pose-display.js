...
const PoseDisplay = (() => {
  ...
  function create() {
    ...
    function push(pose, visibility, now) {
      if (lastSample !== null) interval = interval * 0.7 + Math.min(100, now - lastSample) * 0.3;
      lastSample = now;
    ...
    function sample(now) {
      ...
      for (const [key, j] of Object.entries(joints)) {
        pose[key] = at(j, now);
        visibility[key] = j.visible && now - j.lastGood <= 200 ? 1 : 0;
        // Keep stationary display points only long enough to fade out.
        if (!visibility[key] && now - j.lastGood > 1000) delete joints[key];
      }
