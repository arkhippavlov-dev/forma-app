...
function makeSnapshot(detectorResult, pose, visibility, t, metadata) {
  return {
    t: t,
    ...
    rawVisibility: cloneVisibility(metadata?.rawVisibility),
    aspectRatio: detectorResult.aspectRatio || 1,
    displayPose: clonePose(metadata?.displayPose),
    displayVisibility: cloneVisibility(metadata?.displayVisibility)
  };
}
  ...
  function buildCompletedRep(rep, endTime) {
    return {
      ...
      truncated: rep.frameCount > rep.frames.length && rep.frames.length >= CONFIG.maxFramesPerRep,
   ...
   * visibility: accepted landmark reliability for analysis, never display opacity.
   * metadata: optional rawVisibility, displayPose and displayVisibility snapshots.
   ...
   */
  function update(detectorResult, pose, visibility, timestamp, metadata) {
    ...
    if (!currentRep && currState && currState !== 'standing') {
      const lead = lastStandingSnapshot && t - lastStandingSnapshot.t <= 250 ? lastStandingSnapshot : null;
      currentRep = startRep(lead ? lead.t : t);
      if (lead) currentRep.frames.push(lead);
    }
    ...
    if (currentRep) {
      ...
      // mode-independent: the squat detector supplies normalized hip descent
      // in both full and partial modes. RepCapture therefore stays reusable and does not need
      ...
      if (bottomMetric !== null &&
        ...
        currentRep.bottomFrame = makeSnapshot(dr, pose, visibility, t, metadata);
      }
  ...
  makeSnapshot(
    ...
    metadata
      ...
      if (currReps > lastKnownReps) {
        ...
        // be double-completed.
        const ending = makeSnapshot(dr, pose, visibility, t, metadata);
        if (currentRep.frames[currentRep.frames.length - 1]?.t !== t) {
          if (currentRep.frames.length >= CONFIG.maxFramesPerRep) currentRep.frames.pop();
          currentRep.frames.push(ending);
        }
    ...
    if (currState === 'standing' && dr.trackingAvailable &&
        (!lastStandingSnapshot || t - lastStandingSnapshot.t >= 1000 / CONFIG.sampleRateHz)) {
      lastStandingSnapshot = makeSnapshot(dr, pose, visibility, t, metadata);
    }
