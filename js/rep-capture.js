...
const RepCapture = (function () {
  ...
  function cloneVisibility(visibility) {
    ...
    for (const key in visibility) {
      if (!Object.prototype.hasOwnProperty.call(visibility, key)) continue;
  ...
  function makeSnapshot(detectorResult, pose, visibility, t) {
    return {
      ...
      hipDropRatio: numOrNull(detectorResult.hipDropRatio),
      pose: clonePose(pose),
  ...
  function buildCompletedRep(rep, endTime) {
    return {
      ...
      maxHipDropRatio: finiteOrNull(rep.maxHipDropRatio),
      bottomFrame: rep.bottomFrame,   // the precise min-kneeAngle snapshot, tracked every call
  ...
  function update(detectorResult, pose, visibility, timestamp) {
    ...
    if (currentRep) {
      ...
      // Precise running min/max — updated every call, independent of the
      // throttled `frames` timeline below, so depth data isn't lost to sampling.
      const hipDropRatio = numOrNull(dr.hipDropRatio);

      // Keep the old knee min/max because the analysis uses them, but choose
      // the actual bottom frame primarily from pelvis descent. This matters
      // when poor knee tracking/valgus makes the 2D knee angle misleading.
      if (ka !== null) {
        if (ka < currentRep.minKneeAngle) {
          currentRep.minKneeAngle = ka;
          if (hipDropRatio === null) {
      ...
      if (hipDropRatio !== null && hipDropRatio > currentRep.maxHipDropRatio) {
        currentRep.maxHipDropRatio = hipDropRatio;
        currentRep.bottomFrame = makeSnapshot(dr, pose, visibility, t);
      }
      if (lka !== null) {
      ...
      if (currentRep.frames.length < CONFIG.maxFramesPerRep &&
        ...
        currentRep.frames.push(makeSnapshot(dr, pose, visibility, t));
  ...
  function getCurrentRep() {
    ...
    return {
      ...
      maxHipDropRatio: finiteOrNull(currentRep.maxHipDropRatio),
