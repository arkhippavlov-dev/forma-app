...
function go(id){
  ...
  if(id !== 's-record'){
    if (recState && !recState.finished) {
      ...
      if (recState.resizeHandler) window.removeEventListener('resize', recState.resizeHandler);
    }
...
async function startRecording(){
  ...
  // A second start or a quick navigation must not leave two camera loops.
  if (recState) {
    recState.finished = true;
    recState.live?.stop();
    clearInterval(recState.timerInt);
  ...
  try{
    ...
    const cameraOK = await attachCamera(
      video,
      'rec',
      'user',
      'recnoperm'
    );
    if (!cameraOK || currentScreen !== 's-record') {
      stopCameraStream(streamRegistry.rec);
      return;
    }
    ...
    recState = {
     ...
     PoseEstimationService.reset();
     SkeletonRenderer.reset();
     window.__formaSquatResult = null;
     window.__formaCaptureResult = null;
     window.__formaPerformance = null;
...
if (repCountEl) {
  repCountEl.textContent = '0';
    ...
    recState.live = PoseLive.start(video, canvas, processPoseMeasurement);
...
function processPoseMeasurement(stableKeypoints, stableVisibility, timestampMs) {
  if (!recState || recState.finished) return;
  const squatResult = SquatDetector.update(stableKeypoints, stableVisibility, timestampMs);
  window.__formaSquatResult = squatResult;
  const captureResult = RepCapture.update(squatResult, stableKeypoints, stableVisibility, timestampMs);
  window.__formaCaptureResult = captureResult;
  if (captureResult.newRep) {
    window.__formaLastSquatAnalysis = SquatAnalysisService.analyze(captureResult.newRep);
  }
  const repCountEl = document.getElementById('repCount');
  if (repCountEl && repCountEl.textContent !== String(squatResult.reps)) {
    repCountEl.textContent = squatResult.reps;
  }
  // Only real, unique observations enter analysis. Render interpolation is
  // deliberately isolated in PoseDisplay, never saved in this buffer.
  if (squatResult.trackingAvailable && recState.lastStoredVideoTime !== timestampMs) {
    recState.lastStoredVideoTime = timestampMs;
    if (!recState.repFrames[0]) recState.repFrames[0] = [];
    recState.repFrames[0].push({ keypoints: stableKeypoints, visibility: stableVisibility });
  }
}
...
function stopRecording(){
  ...
  if (recordingStartPending || !recState || recState.finished) {
    go('s-home');
    return;
  }
