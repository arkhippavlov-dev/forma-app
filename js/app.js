...
function updateRecordingReadiness(result){
  ...
  let text = 'Встань в кадр — можно начинать сразу';
  let status = 'waiting';
  ...
  if (result?.trackingAvailable) {
    ...
    text = result.reps > 0 || result.state !== 'standing'
      ...
      : 'Можно начинать — останавливаться не нужно';
  } else if (result) {
    text = 'Не вижу достаточно точек — поправь положение камеры';
  }
...
squatHud.innerHTML = `
  ...
  TRACKING: ${squatResult.trackingAvailable ? 'yes' : 'no'}<br>
...
`;
    ...
    if (squatResult.trackingAvailable &&
