...
function renderRealSquatResults(squatSet){
  ...
  document.getElementById(
    ...
    <div class="strength-row" style="color:var(--text-dim); line-height:1.5;">
      ${insights.tracking || 'FORMA оценивает выводы только по тем данным, которые удалось уверенно отследить.'}
    </div>
  `;
  ...
  repList.innerHTML =
    repetitions.length
      ? repetitions.map(rep => {
          ...
          const summary =
            rep.interpretation?.summary ||
            'Нажми, чтобы открыть измерения этого повторения.';
          ...
          return `
            ...
            >
              ...
              <div style="flex:1; min-width:0;">
                ...
                <div style="font-size:11.5px; color:var(--text-dim); margin-top:5px; line-height:1.45; font-family:var(--font-mono);">
                  DEPTH ${depth} · ${time} · L/R ${lr}
                </div>
              </div>
  ...
  fatigueNote.textContent =
    ...
    (repetitions.length >= 2
      ? 'FORMA сравнила первый и последний повтор, но заметного изменения по текущим метрикам не обнаружено.'
  ...
  document.getElementById(
    ...
    repetitions.length
      ? (insights.mainFocus || 'FORMA проанализировала повторения и сравнила их между собой.')
...
function openRealSquatRepDetail(repIndex){
  ...
  document.getElementById(
    ...
    <div class="quotebox" style="margin-top:16px;">
      <b>Вывод FORMA</b><br><br>
      ${rep.interpretation?.summary || 'По доступным данным повтор близок к среднему для этого подхода.'}<br><br>
      <span style="color:var(--text-dim);">
        ${rep.interpretation?.tracking || 'Качество отслеживания не определено.'}
      </span>
    </div>

    <div style="font-size:11.5px; color:var(--text-faint); line-height:1.5; margin-top:12px;">
      Сравнения «глубже / мельче», «быстрее / медленнее» и «стабильнее» относятся только к другим повторениям этого же подхода. Это пока не универсальная оценка техники.
    </div>
  `;
