...
function scheduleRecordFrame(){
  ...
  // requestVideoFrameCallback freezes after the first callback for some
  // iPhone/Safari getUserMedia streams. RAF is reliable there; inference is
  // explicitly capped below so it does not run at the screen refresh rate.
...
function recordLoop(timestamp){
  ...
  if (pageHidden || video.readyState < 2 ||
      timestamp - recState.lastInferenceTime < 1000 / LOOP_FPS_CAP) {
    scheduleRecordFrame();
    return;
  }
  recState.lastInferenceTime = timestamp;
  const canvas = document.getElementById('reccanvas');
  const ctx = canvas.getContext('2d');
  try {
    const frame = PoseEstimationService.estimateFrame(video);
