...
function updateRecordingReadiness(result){
  const hint = document.getElementById('recReadiness');
  if (!hint) return;
  let text = 'Ищу человека в кадре…';
  let status = 'waiting';
  if (result?.trackingAvailable) {
    status = 'ready';
    text = '';
  } else if (recState && Date.now() - recState.startTime > 5000) {
    text = 'Пока не вижу человека. Попробуй изменить ракурс камеры.';
  }
  hint.hidden = status === 'ready';
...
async function startRecording(){
  ...
  try{
    ...
    hint.textContent = 'Загружаю распознавание…';
...
if (repCountEl) {
  repCountEl.textContent = '0';
    ...
    const hint = document.getElementById('recReadiness');
    hint.hidden = false;
    hint.textContent = 'Распознавание не запустилось. Вернись назад и повтори запись.';
...
updateRecordingReadiness(squatResult);
...
if (repCountEl) {
  ...
  recState.retryAt = 0;
    ...
    if (hint) {
      hint.hidden = false;
      hint.dataset.state = 'waiting';
      hint.textContent = recState.frameErrors >= 3
        ? 'Сбой распознавания. Останови запись и запусти снова.'
        : 'Восстанавливаю распознавание…';
    }
