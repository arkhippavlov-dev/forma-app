...
async function startRecording(){
  ...
  try{
    ...
    PoseEstimationService.resetVideo();
...
function recordLoop(){
  ...
  // Keep the camera loop alive even if one frame throws. The timer alone
  // must never make a stopped pose loop look like a functioning recording.
  try {
  const now = performance.now();
  if (recState.retryAt && now < recState.retryAt) return;
  ...
  // Safari may settle the layout after recording has begun.
...
const stableVisibility = PoseStabilizer.getVisibility();

// Per-frame diagnostics for troubleshooting without overlays. A missing
// pose from the model differs from points rejected by the stabilizer.
window.__formaTracking = {
  modelPoints: Object.values(frame?.keypoints || {}).filter(Boolean).length,
  visiblePoints: Object.values(stableKeypoints || {}).filter(Boolean).length,
  reliablePoints: Object.values(stableVisibility).filter(v => v >= 0.35).length,
...
// Counting and analysis must never pause detection/drawing of a partial
// skeleton. They can be unavailable while the visible body keeps moving.
try {
...
if (repCountEl) {
  repCountEl.textContent = squatResult.reps;
    ...
    // Preparation and missing tracking are not exercise frames. Repeated
    // renders of the same video frame must not grow the recording buffer.
    if (squatResult.ready && squatResult.trackingAvailable &&
        recState.lastStoredVideoTime !== video.currentTime) {
      recState.lastStoredVideoTime = video.currentTime;
  ...
  recState.movementErrors = 0;
} catch (error) {
  recState.movementErrors = (recState.movementErrors || 0) + 1;
  window.__formaMovementError = { message: error.message, at: Date.now() };
  if (recState.movementErrors === 1) console.error('FORMA movement error:', error);
  // Cancel only the unfinished cycle, preserving completed reps.
  try {
    SquatDetector.resetTracking();
    const videoTime = video.currentTime * 1000;
    RepCapture.update(SquatDetector.update(null, null, videoTime), null, null, videoTime);
  } catch (_) {
    // A broken analysis module must not interrupt the independent renderer.
  }
}

  recState.frameErrors = 0;
  recState.retryAt = 0;
  } catch (error) {
    recState.frameErrors = (recState.frameErrors || 0) + 1;
    recState.retryAt = performance.now() + 500;
    window.__formaPoseError = { message: error.message, at: Date.now() };
    if (recState.frameErrors === 1) console.error('FORMA frame error:', error);
    // An incomplete cycle across an inference failure is not a repetition.
    SquatDetector.resetTracking();
    const videoTime = document.getElementById('recvideo').currentTime * 1000;
    const lost = SquatDetector.update(null, null, videoTime);
    RepCapture.update(lost, null, null, videoTime);
  } finally {
    if (!recState.finished) recState.raf = requestAnimationFrame(recordLoop);
  }
}
          ...
          AnalysisAPI
            ...
            .then(
              result => {
                ...
                if (result.repetitions > 0 && Number.isFinite(result.score)) {
...
function renderResults(result){
  ...
  const hasDepthMeasurement =
    result.exercise === 'squat' &&
    (result.availability?.depth || 0) > 0;
  ...
  document.getElementById(
    ...
    `conic-gradient(
      ${c}
      ${(hasScore ? result.score : 0) * 3.6}deg,
      var(--surface-2) 0deg
    )`;
  ...
  lbl.textContent =
    !result.repetitions
      ? '⚪ Подход не распознан'
      : !hasScore
        ? '⚪ Недостаточно данных для балла'
      : hasDepthMeasurement
        ? result.errors.some(error => error.key === 'depth')
          ? '🟡 Глубина требует внимания'
          : '🟢 Оценена глубина'
      :
    result.score >= 80
      ? '🟢 Хорошо'
  ...
  lbl.style.color =
    hasScore ? c : 'var(--text-dim)';


  if (!result.repetitions) {

    document.getElementById('subscoreWrap').innerHTML =
      '<div class="strength-row">Нет повторов для оценки.</div>';
    document.getElementById('issuesWrap').innerHTML = '';
    document.getElementById('strengthsWrap').innerHTML =
      '<div class="strength-row">Поставь телефон так, чтобы было видно движение, и запиши подход заново.</div>';
    ...
    document.getElementById('compareWrap').innerHTML =
      '<div class="sub">Траектория появится после первого распознанного повтора.</div>';
    document.getElementById('mainRecommendation').textContent =
      'Поставь телефон устойчиво, дождись готовности и запиши подход заново.';
    return;
  }
  ...
  subWrap.innerHTML =
    order
      .filter(
        k =>
          Number.isFinite(result.sub_scores[k])
      )
      .map(
        k => {
  ...
  if (!subWrap.innerHTML) {
    subWrap.innerHTML =
      '<div class="strength-row">Пока недостаточно данных для подоценок.</div>';
  }
    ...
    issuesWrap.innerHTML = `
      ...
      <div class="issue-card good">
        ...
        <div class="issue-head">
          🟢 ${hasDepthMeasurement ? 'По глубине замечаний нет' : 'Существенных ошибок не найдено'}
        </div>

        <div class="issue-body">
          ${hasDepthMeasurement
            ? 'Остальные параметры техники пока не оцениваются.'
            : 'Продолжай в том же темпе и постепенно наращивай нагрузку.'}
        </div>
  ...
  strengthsWrap.innerHTML =
    goods.length
      ...
      : `
        <div class="strength-row">
          ${hasDepthMeasurement
            ? '◌ Пока оценена только глубина; другие параметры добавим отдельно.'
        ...
        r => {
          ...
          return `
            ...
            >
              ...
              <div class="rep-measurement">
                ${r.analysis?.depthResult?.available
                  ? 'Глубина: ' + r.analysis.depthResult.label
                  : 'Глубина: недостаточно данных'}
              </div>
  ...
  document.getElementById(
    ...
    <div class="compare-col">
      ...
      ${best.snapshot ? `<img src="${best.snapshot}">` : '<div class="compare-placeholder">Траектория доступна в проигрывателе ниже</div>'}
      ...
      >
        Лучшее —
        повтор ${best.index}
        (${Number.isFinite(best.score) ? best.score : '—'})
      </div>
    ...
    <div class="compare-col">
      ...
      ${worst.snapshot ? `<img src="${worst.snapshot}">` : '<div class="compare-placeholder">Траектория доступна в проигрывателе ниже</div>'}
      ...
      >
        Худшее —
        повтор ${worst.index}
        (${Number.isFinite(worst.score) ? worst.score : '—'})
      </div>
...
function openRepDetail(repIndex){
  ...
  const hasRepScore =
    Number.isFinite(rep.score);

  const c =
    scoreColor(
      hasRepScore ? rep.score : 0
    );
  ...
  let issuesHtml =
    rep.issues.length
      ? ''
      : rep.analysis?.depthResult?.available
        ? '<div class="strength-row">🟢 По глубине — без замечаний</div>'
        : '<div class="strength-row">◌ Недостаточно данных для оценки глубины</div>';
  ...
  document.getElementById(
    ...
    <div class="score-hero">
      ...
      <div
        ...
        style="
          ...
          conic-gradient(
            ${c}
            ${(hasRepScore ? rep.score : 0) * 3.6}deg,
            var(--surface-2) 0deg
          )
        "
      >
    ...
    ${
      rep.analysis?.depthResult
        ? `<div class="card" style="margin-bottom:16px;">
             <b>Глубина</b><br>
             ${rep.analysis.depthResult.available
               ? rep.analysis.depthResult.label + ' · надёжность ' + Math.round(rep.analysis.depthResult.confidence * 100) + '%'
               : 'Не определена: в нижней точке не хватило видимых суставов.'}
           </div>`
        : ''
    }
...
function compareLoop(ts){
  ...
  if(frames.length === 0){
    ...
    if(
      ...
      if (typeof PoseEstimationService.idealKeypointsAt === 'function') {
