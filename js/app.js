...
function scorePillClass(score){
  if (!Number.isFinite(score)) return 'pill-neutral';
...
function scoreColor(score){
  if (!Number.isFinite(score)) return getCss('--text-dim');
...
// ---------------- navigation ----------------
function go(id){
  if (currentScreen === 's-record' && id !== 's-record') stopTrackingLoop();
...
async function attachCamera(videoEl, key, facing, noPermId){
  const requestedScreen = currentScreen;
  ...
  for(const constraint of attempts){
    ...
    try{
      ...
      // A permission prompt may resolve after the user has left this screen.
      if (currentScreen !== requestedScreen) { stopCameraStream(stream); return false; }
...
function renderHome(){
      ...
      .forEach(e => {
        ...
        row.innerHTML = `
          ...
          <div class="scorepill ${scorePillClass(e.last.score)}">
            ${Number.isFinite(e.last.score) ? e.last.score : (e.last.fullResult?.repetitions || 0) + ' повт.'}
          </div>
        `;
  ...
  Object.values(HISTORY).forEach(
    arr => {
      ...
      if(arr.length > 1 && Number.isFinite(arr[0].score) && Number.isFinite(arr[arr.length - 1].score)){
  ...
  document.getElementById('progressNum').textContent =
    ...
    pct +
    '%';
  if (!deltaCount) document.getElementById('progressNum').textContent = '—';
        ...
        arr.length
          ...
          ? `
            ...
            <div class="meta">
              ...
              <span>
                Прогресс:
                ${arr.map(a => Number.isFinite(a.score) ? a.score : (a.fullResult?.repetitions || 0) + ' повт.').join(' → ')}
              </span>
            </div>
...
function runFrameCheck(){
  // These are setup tips, not measurements. No artificial recognition timer.
  document.getElementById('beginAnalysisBtn').disabled = false;
}
...
function updateRecordingHint(displayPose){
  const hint = document.getElementById('recReadiness');
  if (!hint) return;
  // Display is independent of exercise readiness and whole-body visibility.
  hint.hidden = Object.values(displayPose || {}).some(p => Array.isArray(p));
  hint.textContent = 'Встань в кадр — можно начинать';
}
...
function stopTrackingLoop(){
  ...
  if (recState.videoCallback != null && video.cancelVideoFrameCallback) {
    video.cancelVideoFrameCallback(recState.videoCallback);
  }
  clearInterval(recState.timerInt);
  if (recState.resize) window.removeEventListener('resize', recState.resize);
}

function scheduleRecordFrame(){
  if (!recState || recState.finished || currentScreen !== 's-record') return;
  const video = document.getElementById('recvideo');
  if (video.requestVideoFrameCallback) {
    recState.videoCallback = video.requestVideoFrameCallback((now, metadata) => recordLoop(metadata.mediaTime * 1000));
  } else {
    recState.raf = requestAnimationFrame(() => recordLoop(video.currentTime * 1000));
  }
}
...
async function startRecording(){
  ...
  try{
    ...
    updateRecordingHint(null);
    document.getElementById('recTimer').textContent = '00:00';
    document.getElementById('forma-squat-hud').textContent = '';
    ...
    const cameraAttached = await attachCamera(
      video,
      'rec',
      'user',
      'recnoperm'
    );
    if (!cameraAttached || currentScreen !== 's-record') return;
    ...
    if (currentScreen !== 's-record') return;
    PoseEstimationService.resetFrameClock();
...
function recordLoop(timestamp){
  if (!recState || recState.finished || currentScreen !== 's-record') return;
  const video = document.getElementById('recvideo');
  if (pageHidden || video.readyState < 2 || recState.lastFrameTime === timestamp) {
    scheduleRecordFrame();
    return;
  }
  recState.lastFrameTime = timestamp;
  const canvas = document.getElementById('reccanvas');
  const ctx = canvas.getContext('2d');
  try {
    const frame = PoseEstimationService.estimateFrame(video, timestamp);
    const displayPose = PoseStabilizer.update(frame?.keypoints || null, frame?.visibility || null, timestamp);
    const visibility = PoseStabilizer.getVisibility();
    const analysisPose = PoseStabilizer.getAnalysisPose();
    const displayVisibility = PoseStabilizer.getDisplayVisibility();
    window.__formaVideoWidth = video.videoWidth;
    window.__formaVideoHeight = video.videoHeight;
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    SkeletonRenderer.drawUser(ctx, canvas.width, canvas.height, displayPose, {}, displayVisibility);
    updateRecordingHint(displayPose);

    if (currentExercise.id === 'squat') {
      const dr = SquatDetector.update(analysisPose, visibility, timestamp, {
        aspectRatio: video.videoWidth / video.videoHeight
      });
      const capture = RepCapture.update(dr, analysisPose, visibility, timestamp, {
        rawVisibility: frame?.visibility, displayPose, displayVisibility
      });
      window.__formaSquatResult = dr;
      window.__formaCaptureResult = capture;
      if (capture.newRep) {
        const analysis = SquatAnalysisService.analyze(capture.newRep);
        recState.reps.push(analysis);
        window.__formaLastSquatAnalysis = analysis;
      }
      document.getElementById('repCount').textContent = dr.reps;
      if (timestamp - recState.lastHudTime >= 100) {
        recState.lastHudTime = timestamp;
        updateSquatHud(dr, capture);
      }
    } else {
      document.getElementById('forma-squat-hud').textContent = `FORMA · TRACKING
Анализ упражнения
ещё не подключён`;
    }
  } catch (error) {
    console.error('FORMA tracking frame:', error);
    document.getElementById('forma-squat-hud').textContent = `Сбой отслеживания
Попробуй начать заново`;
  }
  scheduleRecordFrame();
}

function updateSquatHud(dr, capture){
  const last = capture.completedReps[capture.completedReps.length - 1];
  const a = window.__formaLastSquatAnalysis;
  const depth = a?.depth;
  const hud = document.getElementById('forma-squat-hud');
  const text = `FORMA SQUAT
REPS / CAPTURED  ${dr.reps} / ${capture.completedReps.length}
STATE  ${dr.state}
KNEE  ${Number.isFinite(dr.kneeAngle) ? Math.round(dr.kneeAngle) + '°' : '—'}
MODE  ${dr.mode}
LAST REP  ${last?.index || '—'}
TIME  ${last ? (last.durationMs / 1000).toFixed(2) + 's' : '—'}
DEPTH  ${depth?.available ? depth.depthValue.toFixed(2) + (depth.method === 'torso' ? ' T' : ' S') : '—'}
...
async function runAnalysisPipeline(){
  document.getElementById('analyzingStep').textContent = 'Собираю анализ повторений…';
  const video = document.getElementById('recvideo');
  try {
    const result = await AnalysisAPI.analyzeVideo({
      exerciseId: currentExercise.id,
      completedReps: recState ? RepCapture.getCompletedReps() : [],
      repRecords: recState?.reps || [],
      videoWidth: video.videoWidth || window.__formaVideoWidth,
      videoHeight: video.videoHeight || window.__formaVideoHeight
    });
    ...
    console.error('FORMA analysis:', error);
    document.getElementById('analyzingStep').textContent = 'Не удалось собрать анализ: ' + error.message;
  }
}
...
function renderResults(result){
  ...
  lastResult = result;
  if (result.meta?.source === 'squat-2d-v1') { renderSquatResults(result); return; }
  document.querySelector('#s-results .score-hero').hidden = false;
  document.getElementById('resultContext').hidden = true;
...
function openRepDetail(repIndex){
  if (lastResult.meta?.source === 'squat-2d-v1') { openSquatRepDetail(repIndex); return; }
...
function openCompareScreen(){
  ...
  go('s-compare');
  const hasIdeal = typeof PoseEstimationService.idealKeypointsAt === 'function';
  document.querySelectorAll('#s-compare .mode-btn').forEach(b => {
    b.hidden = b.dataset.mode !== 'mine' && !hasIdeal;
  });
  if (lastResult.meta.videoWidth && lastResult.meta.videoHeight) {
    window.__formaVideoWidth = lastResult.meta.videoWidth;
    window.__formaVideoHeight = lastResult.meta.videoHeight;
  }
...
function setCompareMode(mode){
  if (mode !== 'mine' && typeof PoseEstimationService.idealKeypointsAt !== 'function') return;
...
function renderCompareLegend(mode){
  if (lastResult?.meta?.source === 'squat-2d-v1') {
    document.getElementById('compareLegend').textContent = 'Наблюдаемая траектория · цвет не оценивает технику';
    return;
  }
...
function compareLoop(ts){
  ...
  if(frames.length === 0){
    ...
    if (Number.isFinite(frames[0].t)) {
      if (compareState.playStart == null) compareState.playStart = ts;
      const duration = Math.max(100, frames[frames.length - 1].t - frames[0].t + 100);
      const target = frames[0].t + ((ts - compareState.playStart) % duration);
      compareState.i = 0;
      while (compareState.i + 1 < frames.length && frames[compareState.i + 1].t <= target) compareState.i++;
    }
    const f =
      frames[
        compareState.i %
    ...
    if(
      ...
      if (typeof PoseEstimationService.idealKeypointsAt !== 'function') return;
            ...
            a =>
              ...
              <div class="trend-row">
                ...
                <span
                  ...
                  "
                >
                  ${Number.isFinite(a.score) ? a.score + '/100' : (a.fullResult?.repetitions || 0) + ' повт.'}
                </span>
