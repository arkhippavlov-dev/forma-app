...
function updateRecordingReadiness(result){
  ...
  if (!hint) return;
  let text = 'Встань прямо и замри на секунду';
  let status = 'waiting';
  if (result?.ready && !result.trackingAvailable) {
    text = 'Не вижу движение — подожди';
  } else if (result?.ready) {
    status = 'ready';
    text = result.reps > 0 || result.state !== 'standing'
      ? 'Идёт подсчёт повторов'
      : 'Можно начинать';
  } else if (result?.readiness === 'stabilizing') {
    text = 'Проверяю исходное положение…';
  }
...
function renderResults(result){
  ...
  if (!result.repetitions) {
    ...
    document.getElementById('strengthsWrap').innerHTML =
      '<div class="strength-row">Проверь, что тело видно целиком, дождись «Можно начинать» и повтори подход.</div>';
