...
function runAnalysisPipeline(){
  ...
  const steps = isRealSquat
    ? [
        'Собираю отдельные повторения…',
        'Анализирую глубину…',
        'Сравниваю левую и правую сторону…',
        'Анализирую темп движения…',
        'Проверяю положение корпуса…',
        'Собираю результат подхода…'
      ]
    : [
...
function renderRealSquatResults(squatSet){
  ...
  document.getElementById(
    ...
    repetitions.length
      ? [
          ...
          realSquatMetricRow(
            'Разница L/R колена',
            Number.isFinite(averages.kneeAngleDifference)
              ? formatNumber(averages.kneeAngleDifference, 1) + '°'
              : '—'
          ),
          realSquatMetricRow(
            'Изменение наклона корпуса',
            Number.isFinite(averages.torsoRangeDegrees)
              ? formatNumber(averages.torsoRangeDegrees, 1) + '°'
              : '—'
          )
        ].join('')
      : '<div style="font-size:13px; color:var(--text-dim);">Завершённых повторений не найдено.</div>';


  document.getElementById(
    'issuesTitle'
  ).textContent =
    'Что видно по подходу';


  const depthEntries =
    metricEntries(
      repetitions,
      rep => rep.depth?.depthValue
    );

  const durationEntries =
    metricEntries(
      repetitions,
      rep => rep.tempo?.totalMs
    );

  const asymmetryEntries =
    metricEntries(
      repetitions,
      rep => rep.symmetry?.kneeAngleDifference
    );

  const torsoEntries =
    metricEntries(
      repetitions,
      rep => rep.torso?.rangeDegrees
    );

  const deepest = maxEntry(depthEntries);
  const shallowest = minEntry(depthEntries);
  const fastest = minEntry(durationEntries);
  const slowest = maxEntry(durationEntries);
  const mostStableTorso = minEntry(torsoEntries);
  ...
  if(deepest && shallowest){
    cards.push(
      realSquatInfoCard(
        'Глубина повторений',
        `DEPTH менялся от ${formatNumber(shallowest.value, 3)} до ${formatNumber(deepest.value, 3)}. ` +
        `Самый глубокий по текущей 2D-метрике — повтор ${deepest.index}, ` +
        `наименьшая глубина — повтор ${shallowest.index}.`
      )
    );
  }
  ...
  if(fastest && slowest){
    cards.push(
      realSquatInfoCard(
        'Темп',
        `Полное время повторения менялось от ${formatSeconds(fastest.value)} до ${formatSeconds(slowest.value)}. ` +
        `Самый быстрый — повтор ${fastest.index}, самый медленный — повтор ${slowest.index}.`
      )
    );
  }

  if(asymmetryEntries.length){
    cards.push(
      realSquatInfoCard(
        'Левая / правая сторона',
        `Средняя разница минимального угла колена между сторонами — ` +
        `${formatNumber(averages.kneeAngleDifference, 1)}°. ` +
        `Это описательная 2D-метрика: ракурс камеры может заметно влиять на значение.`
      )
    );
  }
  ...
  if(mostStableTorso){
    cards.push(
      realSquatInfoCard(
        'Корпус',
        `Наименьшее изменение наклона корпуса внутри одного повторения было в повторе ` +
        `${mostStableTorso.index}: ${formatNumber(mostStableTorso.value, 1)}°. ` +
        `Более вертикальный корпус сам по себе не считается автоматически лучшей техникой.`
      )
    );
  }
  ...
  document.getElementById(
    ...
    cards.length
      ...
      : realSquatInfoCard(
          'Пока мало данных',
          'Нужен хотя бы один полностью завершённый повтор с достаточным количеством видимых суставов.'
        );
  ...
  document.getElementById(
    ...
    <div class="strength-row">
      Отслеживание: ${formatPercent01(averages.confidence)}
    </div>
    <div class="strength-row" style="color:var(--text-dim); line-height:1.5;">
      Сейчас FORMA показывает реальные измеренные значения по каждому повторению. Жёсткие универсальные оценки «хорошо / плохо» для этих метрик пока не применяются.
    </div>
  `;
  ...
  repList.innerHTML =
    repetitions.length
      ? repetitions.map(rep => {
          ...
          const lr =
            Number.isFinite(rep.symmetry?.kneeAngleDifference)
              ? rep.symmetry.kneeAngleDifference.toFixed(1) + '°'
              : '—';

          const conf =
            formatPercent01(rep.dataQuality?.confidence);
          ...
          return `
            ...
            >
              ...
              <div style="flex:1; min-width:0;">
                ...
                <div style="font-size:11.5px; color:var(--text-dim); margin-top:4px; line-height:1.45;">
                  ${time} · L/R ${lr}
                </div>
              </div>
  ...
  if(repetitions.length >= 2){
    ...
    const depthDelta =
      Number.isFinite(first.depth?.depthValue) &&
      Number.isFinite(last.depth?.depthValue)
        ? last.depth.depthValue - first.depth.depthValue
        : null;

    const timeDelta =
      Number.isFinite(first.tempo?.totalMs) &&
      Number.isFinite(last.tempo?.totalMs)
        ? last.tempo.totalMs - first.tempo.totalMs
        : null;
    ...
    if(Number.isFinite(depthDelta)){
      pieces.push(
        `DEPTH: ${depthDelta >= 0 ? '+' : ''}${depthDelta.toFixed(3)}`
      );
    }

    if(Number.isFinite(timeDelta)){
      pieces.push(
        `время: ${timeDelta >= 0 ? '+' : ''}${(timeDelta / 1000).toFixed(2)} c`
      );
    }

    fatigueNote.textContent =
      pieces.length
        ? `Первый → последний повтор: ${pieces.join(' · ')}.`
        : 'Недостаточно сопоставимых данных для оценки изменения по ходу подхода.';

  }else{
    fatigueNote.textContent =
      'Для сравнения динамики подхода нужно минимум два завершённых повторения.';
  }
  ...
  const recommendationTitle =
    document.getElementById('mainRecommendationTitle');
  ...
  if(deepest){
    summaryParts.push(
      `самый глубокий повтор — ${deepest.index}`
    );
  }

  if(mostStableTorso){
    summaryParts.push(
      `наименьшее изменение корпуса — повтор ${mostStableTorso.index}`
    );
  }
  ...
  document.getElementById(
    ...
    repetitions.length
      ? `FORMA отдельно проанализировала ${repetitions.length} повторений` +
        (summaryParts.length ? `: ${summaryParts.join(', ')}.` : '.')
      : 'Завершённых повторений для анализа не найдено.';
}
...
function openRealSquatRepDetail(repIndex){
  ...
  const rows = [
    ...
    realSquatMetricRow(
      'Вниз',
      formatSeconds(rep.tempo?.descentMs)
    ),
    realSquatMetricRow(
      'Вверх',
      formatSeconds(rep.tempo?.ascentMs)
    ),
    realSquatMetricRow(
      'Разница L/R колена',
      Number.isFinite(rep.symmetry?.kneeAngleDifference)
        ? formatNumber(rep.symmetry.kneeAngleDifference, 1) + '°'
        : '—'
    ),
    realSquatMetricRow(
      'Корпус в нижней точке',
      Number.isFinite(rep.torso?.bottomLeanDegrees)
        ? formatNumber(rep.torso.bottomLeanDegrees, 1) + '°'
        : '—'
    ),
    realSquatMetricRow(
      'Изменение корпуса',
  ...
  document.getElementById(
    ...
    <div class="quotebox" style="margin-top:16px;">
      Это измерения конкретного повторения. Пока они используются как описательные данные без универсального вердикта «правильно / неправильно».
    </div>
  `;
