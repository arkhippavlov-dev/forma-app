...
const PoseStabilizer = (function () {
  const CONFIG = {
    ...
    minCutoffHz: 0.9,
    speedGain: 2.6,           // body lengths / second -> adaptive cutoff
    maxCutoffHz: 9,
    // The head marker is visually unforgiving: a few pixels of landmark
    // noise look like shaking when a person is still. Keep it inside a
    // small scale-relative rest zone, but leave the normal responsive
    // filter in charge as soon as it moves beyond that zone.
    headMinCutoffHz: 0.38,
    headSpeedGain: 1.35,
    headMaxCutoffHz: 4.8,
    headRestBodyRatio: 0.016,
    headRestMin: 0.0025,
    headRestMax: 0.007,
    velocityCutoffHz: 2.5,
  ...
  function update(pose, visibility, timestamp) {
    ...
    for (const joint of joints) {
      ...
      // Do not draw the first guess for a joint. At the beginning/end of
      // a recording MediaPipe often returns a single, crooked partial pose
      // while it is still deciding which limb belongs where. Two nearby
      // observations add only ~33–66 ms, but remove that visual snap.
      if (!previous) {
        if (!reliable[joint]) {
          delete acquireCandidates[joint];
          result[joint] = null;
        ...
        const candidate = acquireCandidates[joint];
        const radius = Math.max(0.025, bodyScale * CONFIG.acquireBodyRatio);
        const continues = candidate && now - candidate.lastTime <= CONFIG.acquireGapMs &&
          distance(raw, candidate.point) <= radius;
        const nextCandidate = continues
          ? { point: clonePoint(raw), lastTime: now, count: candidate.count + 1 }
          : { point: clonePoint(raw), lastTime: now, count: 1 };
        acquireCandidates[joint] = nextCandidate;
        if (nextCandidate.count < CONFIG.acquireFrames) {
          result[joint] = null;
          effectiveVisibility[joint] = 0;
          analysisPose[joint] = null;
          displayVisibility[joint] = 0;
      ...
      if (accept) {
        ...
        const measuredVx = velocityOK ? previous.vx + speedAlpha *
          ((raw[0] - previous.raw[0]) / Math.max(age / 1000, 0.001) - previous.vx) : 0;
        const measuredVy = velocityOK ? previous.vy + speedAlpha *
          ((raw[1] - previous.raw[1]) / Math.max(age / 1000, 0.001) - previous.vy) : 0;
        const isHead = joint === 'head';
        const restRadius = clamp(bodyScale * CONFIG.headRestBodyRatio,
          CONFIG.headRestMin, CONFIG.headRestMax);
        // This is display-only. Keep the latest raw point for reliability
        // and outlier checks, while rendering a still face from its stable
        // point. A real movement accumulates past restRadius immediately.
        const headAtRest = isHead && previous && !born &&
          distance(raw, previous.point) <= restRadius;
        const target = headAtRest ? previous.point : raw;
        const vx = headAtRest ? 0 : measuredVx;
        const vy = headAtRest ? 0 : measuredVy;
        // Raw frame-to-frame head jitter can look like high speed. Measure
        // head motion against its displayed point instead, so noise cannot
        // increase the cutoff and leak straight through the smoothing.
        const speed = isHead && previous
          ? distance(target, previous.point) / Math.max(seconds * bodyScale, 0.001)
          : Math.hypot(vx, vy) / bodyScale;
        const quality = clamp(vis[joint] / CONFIG.minVisibility, 0.65, 1);
        const minCutoff = isHead ? CONFIG.headMinCutoffHz : CONFIG.minCutoffHz;
        const speedGain = isHead ? CONFIG.headSpeedGain : CONFIG.speedGain;
        const maxCutoff = isHead ? CONFIG.headMaxCutoffHz : CONFIG.maxCutoffHz;
        const cutoff = clamp(minCutoff + speedGain * speed, minCutoff, maxCutoff) * quality;
        const alpha = alphaFor(cutoff, seconds);
        const recoverUntil = returning ? now + CONFIG.reacquireMs : (previous?.recoverUntil || 0);
        const point = born ? clonePoint(target) : [
          previous.point[0] + (target[0] - previous.point[0]) * alpha,
          previous.point[1] + (target[1] - previous.point[1]) * alpha
        ];
        ...
        const opacity = previous ? Math.min(1, (born ? 0 : previous.opacity) + dt / 130) : 0;
