  ...
  function start(video, canvas, onMeasurement) {
    ...
    function submit(mediaTime, now) {
      ...
      PoseEstimationService.estimateFrame(video, mediaTime).then(frame => {
        ...
        error = '';
        if (frame.capturedAt <= lastAnalysisAt || completedAt - frame.capturedAt > 500) return;
        ...
        display.push(stable, visibility, completedAt);
    ...
    function updateHUD(now) {
      ...
      if (summary) summary.textContent = `FORMA · ${fmt(renderFPS)} FPS`;
      if (!body) return;
      body.textContent = [
    ...
    function draw(now) {
      ...
      if (now - lastResultAt > 300 && now - lastMissingAt > 100) {
        lastMissingAt = now;
        lastAnalysisAt = now;
